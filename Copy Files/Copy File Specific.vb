Sub FileManageSpecific()
      
      Dim folderName As String
      Dim searchLoc As String 'specific locations
      Dim destinLoc As String
      Dim fileName As String
      
      folderName = (Sheets("Sheet2").Range("D5").Value)
      searchLoc = (Sheets("Sheet2").Range("D7").Value)
      
      
      'Part 1 - Uses the inputted folder folder name from the cell and then creates a folder with that name.
      If (folderName <> "") Then
            
            'Part 2 - Uses the specified path of the Shared Drive/whatever folder you are using.
            If (searchLoc = "") Then
                MsgBox "You have not defined a location to search from!"
                Exit Sub
            End If
        'part 1 continued
        If Dir("C:\Users\" & (Environ$("Username")) & "\Desktop\" & folderName, vbDirectory) = "" Then
            MkDir ("C:\Users\" & (Environ$("Username")) & "\Desktop\" & folderName)
        Else
            MsgBox "The folder already exists, please check to make sure your folder name is not wrong or rename your folder!"
            Exit Sub
        End If
      Else
        MsgBox "Your folder requires a name!"
        Exit Sub
      End If
      
      'Part 3 - Copies the files.
      Sheets("Sheet2").Range("A2").Select
      'Set Do loop to stop when an empty cell is reached.
      Do Until IsEmpty(ActiveCell)
         'Search Method
         Call FileSearching(searchLoc)
         'Step down 1 row from present location.
         ActiveCell.Offset(1, 0).Select
      Loop
      
      Call Shell("explorer.exe" & " " & "C:\Users\" & (Environ$("Username")) & "\Desktop\" & folderName, vbNormalFocus)
      MsgBox "The violations have been copied! Please double check the resulting folder to make sure no violations were missed."
      
End Sub

Function FileSearching(sPath As String) As String

    Dim FSO As New FileSystemObject
    Dim myFolder As Folder
    Dim mySubFolder As Folder
    Dim myFile As file
    Dim destinLoc As String
    
    fileName = "?" & ActiveCell.Value & "*" & ".pdf"
    destinLoc = "C:\Users\" & (Environ$("Username")) & "\Desktop\" & Range("D5").Value & "\"
         
    Set myFolder = FSO.GetFolder(sPath)
    
    For Each myFile In myFolder.Files
            If myFile.Name Like fileName Then
                 'To see if such stuff exist Debug.Print myFile.Name & " in " & myFile.Path 'Or do whatever you want with the file
                 'Method to copy the found file
                 FSO.CopyFile myFile, destinLoc
            End If
    Next
       
    For Each mySubFolder In myFolder.SubFolders
        For Each myFile In mySubFolder.Files
            If myFile.Name Like fileName Then
                 FSO.CopyFile myFile, destinLoc
            End If
        Next
        FileSearching = FileSearching(mySubFolder.Path)
    Next
    
End Function